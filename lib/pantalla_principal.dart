            ...
            children: [
                ...
                builder: (context, snapshot) {
                  ...
                  return DropdownButtonFormField<String>(
                    ...
                    items: snapshot.data!.docs.map((doc) => DropdownMenuItem(
                      value: doc.id, 
                      child: Text("${doc['nombre_paciente'] ?? ''} ${doc['apellido_paciente'] ?? ''}")
                    )).toList(),
              ...
              Container(
                ...
                child: _pacienteSeleccionadoId == null 
                  ? const Center(child: Text("Selecciona un paciente para ver sus medicamentos"))
                  : StreamBuilder<QuerySnapshot>(
                      stream: _firestore.collection('pastillas').where('paciente_id', isEqualTo: _pacienteSeleccionadoId).snapshots(),
                      builder: (context, snapshot) {
                        if (!snapshot.hasData) return const CircularProgressIndicator();
                        var docs = snapshot.data!.docs;
                        if (docs.isEmpty) return const Text("No hay medicamentos registrados para este paciente");

                        return Column(
                          children: docs.map((doc) {
                            var data = doc.data() as Map<String, dynamic>;
                            String nombre = data['nombre_pastilla'] ?? 'Sin nombre';
                            int cantRestante = int.tryParse(data['cantidad_restante']?.toString() ?? '0') ?? 0;
                            double pct = (cantRestante / 10).clamp(0.0, 1.0);
                            int pctTexto = (pct * 100).toInt();

                            Color colorBarra = (pctTexto == 0) ? Colors.red : (pctTexto <= 30 ? Colors.orange : Colors.green);

                            return Padding(
                              padding: const EdgeInsets.symmetric(vertical: 12.0),
                              child: Row(
                                children: [
                                  SizedBox(width: 90, child: Text(nombre, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 14), overflow: TextOverflow.ellipsis)),
                                  Expanded(
                                    child: ClipRRect(
                                      borderRadius: BorderRadius.circular(10),
                                      child: LinearProgressIndicator(value: pct, backgroundColor: Colors.grey[100], valueColor: AlwaysStoppedAnimation<Color>(colorBarra), minHeight: 12),
                                    ),
                                  ),
                                  const SizedBox(width: 15),
                                  SizedBox(width: 50, child: Text("$pctTexto%", style: TextStyle(color: colorBarra, fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.right)),
                                ],
                              ),
                            );
                          }).toList(),
